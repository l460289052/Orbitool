# -*- coding: utf-8 -*-
from __future__ import annotations

from typing import Union, Iterator, Tuple, overload

import numpy as np


class Formula:
    @overload
    def __init__(self): ...
    @overload
    def __init__(self, formula: str): ...
    @overload
    def __init__(self, formula: dict): ...
    @overload
    def __init__(self, *, charge: float = ..., **element_num: int): ...

    def __init__(self, formula=None, *, charge=..., **kwargs): ...

    @property
    def charge(self) -> int: ...
    @charge.setter
    def charge(self, value: int): ...
    @property
    def isIsotope(self) -> bool: ...
    def mass(self) -> float: ...
    def dbe(self) -> float: ...
    def findOrigin(self) -> Formula: ...
    def addElement(self, element: str, m: int = 0, num: int = 1): ...
    def toStr(self, showProton: bool = False,
              withCharge: bool = True) -> str: ...

    def absoluteAbundance(self) -> float: ...
    def relativeAbundance(self) -> float: ...
    def clear(self): ...
    def to_numpy(self) -> np.ndarray: ...
    def to_dict(self) -> dict: ...
    @classmethod
    def from_numpy(cls, data: np.ndarray): ...
    def keys(self) -> Iterator[str]: ...
    def items(self) -> Iterator[Tuple[str, int]]: ...

    def atoms(self) -> Formula:
        """return parttern of this formula, Formula('SO4-2').atoms() == Formula('SO-')"""
        pass

    def __setitem__(self, key: str, value: int): ...
    def __getitem__(self, key: str) -> int: ...
    def __len__(self) -> int: ...
    def __str__(self): ...
    def __repr__(self): ...
    def __copy__(self): ...
    def __iadd__(self, formula: Formula) -> Formula: ...
    def __add__(self, formula: Formula) -> Formula: ...
    def __isub__(self, formula: Formula) -> Formula: ...
    def __sub__(self, formula: Formula) -> Formula: ...
    def __imul__(self, times: int) -> Formula: ...
    def __mul__(self, times: int) -> Formula: ...
    def __eq__(self, formula: Formula): ...
    def __contains__(self, formula: Formula): ...
    def __hash__(self): ...
