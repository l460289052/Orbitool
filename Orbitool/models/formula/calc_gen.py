...
class IsotopeNum(BaseRowStructure):
    e_num: int
    i_num: int
    min: int = 0
    max: int = 0
...
class CalcIsotopeNum(BaseRowStructure):
    element: str
    e_num: int
    i_num: int
    global_limit: bool
...
class CalculatorGenerator(BaseStructure):
    ...
    dbe_limit: bool = True
    debug: bool = False
    element_states: Dict[str, State] = {}
    isotope_usable: Dict[str, IsotopeNum] = {}
    ...
    def generate(self, cls: Union[Type[Calculator], None] = None):
        if cls is None:
            ...
            cls = Calculator
        assert len(
            self.isotope_usable) >= 2, "At least two isotopes should be added"
